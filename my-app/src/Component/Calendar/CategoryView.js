      ...
      <div className='categoryView' id='view'>
        <div className='categoryView' id='categoryList'>
          {props.foodDB &&
            props.foodDB.map((category) => (
              <Fragment key={category._id}>
                <button
                  className='categoryView categories'
                  style={{
                    backgroundColor: category.color,
                    color: getTextColor(category.color),
                    fontSize: mySize.adjust(0.02),
                    boxShadow: category.name === curCategory.name ? "rgb(255 188 0) -2px 0px 14px 3px" : "",
                  }}
                  onMouseEnter={() => setDelBtn(category._id)}
                  onMouseLeave={() => setDelBtn("")}
                  onClick={() => setCurCategory({ name: category.name, id: category._id })}
                >
                  {delBtn === category._id && category._id !== "67d8b49fb95b08e413d20fff" && (
                    <button className='categoryView delBtn' id={category._id} onClick={() => delCategory(category._id)} style={{ width: mySize.adjust(0.025) }}>
                      {" "}
                      X{" "}
                    </button>
                  )}
          ...
          <form className='categoryView' id='addCategory' onSubmit={addCategory}>
            ...
            <input type='text' className='categoryView' id='inputCategory' placeholder='+ new category' style={{ fontSize: mySize.adjust(0.02) }} name='name'></input>
          </form>
        </div>
        <div className='categoryView' id='itemList'>
          {curFoods &&
            curFoods.map((item) => (
              <Fragment key={item._id}>
                <button
                  title={item.name}
                  className='categoryView item'
                  id='item_1'
                  style={{ fontSize: mySize.adjust(0.015) }}
                  onMouseEnter={() => setDelBtn(item._id)}
                  onMouseLeave={() => setDelBtn("")}
                >
                  {delBtn === item._id && (
                    <button className='categoryView delBtn' style={{ backgroundColor: "white", color: "black" }} id={item._id} onClick={() => delItem(item._id)}>
                      {" "}
                      X{" "}
                    </button>
                  )}
          ...
          <form className='categoryView' id='addItem' onSubmit={addItem}>
            <input type='text' className='categoryView' id='inputItem' placeholder='+ new item' style={{ fontSize: mySize.adjust(0.015) }} name='name'></input>
          </form>
        </div>
      </div>
    </div>
