  ...
  return (
    <div id='postColumn' style={{ width: isVertical ? "96%" : "" }}>
      {user.title === "admin" && (
        <div style={{ display: "flex", justifyContent: "center", width: "100%" }}>
          <label className='container' style={{ fontSize: mySize.adjust(0.02), paddingLeft: "3.5%", fontWeight: "bold" }}>
            ...
            <span
              className='checkmark'
              style={{ width: mySize.adjust(0.02), height: mySize.adjust(0.02), "--checkmark-width": mySize.adjust(0.004), "--checkmark-height": mySize.adjust(0.009) }}
            ></span>
          </label>
        </div>
      )}
      ...
      <div style={{ display: "flex", alignItems: "center", marginTop: "2%", justifyContent: "center" }}>
        ...
        {((onePost.author && onePost.author[0]._id === user.id) || user.title === "admin") && !isVertical && (
